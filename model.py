...
class GatedCNN(object):
    def __init__(self, conf):
        ...
        embed = self.creat_embeddings(self.X, conf)
        ...
        gradients = trainer.compute_gradient(self.loss)
    ...
    def conv_op(self, fan_in, shape, name):
        W = tf.get_variable('%s_W'%name, shape, tf.float32, tf.random_normal_intializer(0.0, 0.1))
        b = tf.get_variable('%s_b'%name, shape[-1], tf.float32, tf.constant_initializer(1.0))
        return tf.add(tf.nn.conv2d(fan_in, W, strides=[1, 1, 1, ], padding='SAME'), b)


    def create_summaries(self):
        tf.summary.scalar('loss', self.loss)
        tf.summary.scalar('perplexity', self.perplexity)
        self.merged_summary_op = tf.summary_merge_all()
